...
#   
#   
#   File:   project-mcp-tools/cpp/cpp_lib/llm_rules.py
...
_rules_style_guide_dir = Path( __file__ ).resolve( ).parent.parent.parent / "docs" / "agent" / "style-guide"
...
def _resolve_markdown_paths( ) -> list[ str ]:
    custom_path = os.environ.get( "CPP_LLM_RULES_MD" )
    if custom_path:
        return  [ custom_path ]
    return  [
         str( _rules_style_guide_dir / "all.md" )
        ,str( _rules_style_guide_dir / "cpp.md" )
    ]
...
def _load_rules( ) -> list[ llm_rule ]:
    ...
    if _rules_cache is None:
        rules_by_id: dict[ str, llm_rule ] = { }
        for md_path in _resolve_markdown_paths( ):
            for rule in _parse_rules_from_markdown( md_path ):
                if rule.rule_id not in rules_by_id:
                    rules_by_id[ rule.rule_id ] = rule
        _rules_cache = list( rules_by_id.values( ) )
    return  _rules_cache
