    ...
    def link( self ):
        ...
        if self.cpp.is_linkage_needed:
            ...
            if process.returncode != 0:
                ...
                self.project.stop( )
                self.project._build_errors.append( f"linking failed for {self.binary_path}" )
            else:
...
class project_core:
    def __init__( self, config = { } ):
        ...
        self.compiler = cpp_compiler( self.config )
    ...
    def print( self, *args, **kwargs ):
        ...
        with self._lock:
            ...
            self._build_logs.append( text )

    def flush_logs( self ):
        with self._lock:
            if self._build_logs:
                print( "\n".join( self._build_logs ) )
    ...
    def build( self, cpp_file ):
        ...
        if process.returncode != 0:
            self.stop( )
            self._build_errors.append( f"""compiler: {compiler_command}\nfailed: {"\n".join( lines )}\ncompilation failed for {cpp_file.path}""" )
            return
        else:
